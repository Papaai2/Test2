...
try {
    if ($report_type === 'user_balances') {
        $sql_users = "
            SELECT u.id AS user_id, u.full_name, u.email, u.role, d.name AS department_name, u.employee_code,
        ...
        if (!empty($search_query)) {
            $where_clauses[] = "(u.full_name LIKE ? OR u.email LIKE ? OR u.employee_code LIKE ?)"; // Added employee_code to search
        ...
        $sql_users .= " GROUP BY u.id, u.full_name, u.email, u.role, d.name, u.employee_code ORDER BY u.full_name ASC"; // Added u.employee_code to GROUP BY

        $stmt_users = $pdo->prepare($sql_users);
        $stmt_users->execute($sql_params);
        $users_data = $stmt_users->fetchAll();

        // Call the centralized helper function for CSV export
        exportUserBalancesToCsv($users_data, "user_balances_report_" . date('Y-m-d') . ".csv");
